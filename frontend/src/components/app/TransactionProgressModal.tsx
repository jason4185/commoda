...
import type { TransactionProgress, TransactionStage } from "@/lib/commoda/transaction-types";
    ...
    <Dialog open={open} onOpenChange={(value) => canClose && onOpenChange(value)}>
      ...
      >
        ...
        <div className="flex flex-col items-center pt-2 text-center">
          <div className={cn("flex size-14 items-center justify-center rounded-2xl", accepted ? "bg-success/12 text-success" : failed ? "bg-danger/10 text-danger" : "bg-amber/10 text-amber")}>
            {accepted ? <Check className="size-7" /> : failed ? <CircleAlert className="size-6" /> : <Loader2 className="size-6 animate-spin" />}
          </div>
          <h2 className="mt-4 text-2xl font-semibold text-navy-deep">{failed ? "Transaction not accepted" : accepted ? "Accepted" : title}</h2>
          <p className="mt-1.5 max-w-xs text-sm text-slate">{failed ? (progress.error ?? "Something went wrong. You can close this window and try again.") : accepted ? (progress.outcome ?? "The transaction was accepted by GenLayer.") : STEPS[currentIndex]?.hint}</p>
        </div>
        <ol className="mt-6 space-y-1">
          {STEPS.map((step, index) => {
            const state = accepted || index < currentIndex ? "done" : index === currentIndex ? "current" : "todo";

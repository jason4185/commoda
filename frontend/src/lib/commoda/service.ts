...
import type { CancellationStatus, DayResult, DurationDays, DropPct, Market, MarketId, PoolStats, Protection, PurchaseRequest } from "./types";
...
export function getProtectionStatusLabel(state: Protection["state"]) { return { ACTIVE: "Active", CLAIMABLE: "Ready to claim", EXPIRED: "Ended", CLAIMED: "Paid", CANCELLED: "Cancelled" }[state]; }
...
export const commodaService = {
  ...
  async cancelUnresolved(id: string, onProgress?: TransactionProgressCallback) { const owner = getActiveAccount(); if (!owner) throw new Error("Connect your wallet before cancelling."); const readiness = raw(await readContractAsAccount("cancellation_readiness", [BigInt(id)], owner)); if (!readiness.can_cancel) throw new Error("Protection is not ready to cancel and refund yet."); await writeContract("cancel_unresolved_protection", [BigInt(id)], 0n, onProgress); return this.getProtection(id, owner); },
